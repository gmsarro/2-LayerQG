...
# Specify other dynamic parameters
beta=0.2 # nondimensional beta
Ld=1 # deformation radius

#Specify the name of your output directories
sname_u='N128_%s_2.0_0.1_%s.uref1_2.nc'%(str(np.round(Llist[0],2)),str(np.round(Ulist[0],2)))  # output name U
sname_t='N128_%s_2.0_0.1_%s.tref1_2.nc'%(str(np.round(Llist[0],2)),str(np.round(Ulist[0],2)))  # output name T


#################

# read Q_REF
readq=Dataset(datadir+'N128_%s_2.0_0.1_%s.qref1_2.nc'%(str(np.round(Llist[0],2)),str(np.round(Ulist[0],2)))) 
qref=readq.variables['qref1'][:,:].data
tn,yn=np.shape(qref)

# read zonal-mean U and T (used as boundary condition)
readu=Dataset(datadir+'N128_%s_2.0_0.1_%s.nc'%(str(np.round(Llist[0],2)),str(np.round(Ulist[0],2))))
...
#Save both the reference PV and reference temperature
